...
| **10** | Sound — a CC0 chiptune soundtrack driven by the room's phase, stings for the opener and the reveal, a countdown tick, the lobby's one-time sound offer, Music and Sound effects toggles in everybody's toolbox, and a tap-anywhere prompt when a reload leaves the browser holding the sound. [ADR 0044](./adr/0044-every-tab-plays-its-own-copy-of-the-soundtrack.md). | A remote room hears the same bar on every device from the broadcast phase alone; sound is silent until each person turns it on; `e2e/sound.spec.ts` passes under both autoplay policies. | ✅ done |
...
| `?paced=1` | Boots that fixture host-paced: no clock on `brief`, `compose`, `waiting`, `vote` or `tiebreak`, and the host's advance control in the header slot the timer vacates. Same family as `?voting=` and `?format=`, and deliberately one-way — clocks on is already the default ([ADR 0043](./adr/0043-a-paced-room-removes-the-clock-not-the-gate.md)). |
| `?out=2`, `?submitted=2`, `?votes=0` | Hold competitors back from a `waiting` fixture, pre-answer a `compose` one, or cast fewer ballots before a `reveal` — including none, which is the round nobody voted in. Every fixture otherwise submits and votes for everybody, so the faces with someone still missing are unreachable. |
| `?brain=live` | Calls the model for bot jokes instead of the written-in corpus. `stub` is the road the whole suite takes: offline, free, and the same every time. |
