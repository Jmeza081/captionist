...
| `ShortcutHint` | molecule | "Press ⌥ P to pause", beside the clock it pauses. Draws nothing without **both** a keyboard and room for one: `useHasKeyboard` asks `(hover: hover) and (pointer: fine)` because a host in landscape has a wide screen and no keys, and the stylesheet asks for `xl` because a narrow laptop window has keys and no room. The glyph is `⌥` on Apple platforms and `Alt` elsewhere; the visually hidden sentence says "Alt" either way, since that is the key's name |
