...
The lobby's sound offer draws its music glyph on a `$sound-offer-mark` (34px)
tile. The design predates sound, so this is the one metric on the offer that
was chosen rather than copied: big enough to hold an 18px glyph beside a
two-line title and body without setting the card's height.
...
| `TimerPill` | atom | The round clock. Flips to urgent at ≤15s, or on demand for sudden death. `paused` is a third state beside neutral and urgent, and beats urgent: a held clock at nine seconds is not running out, it is stopped. The amber is `$status-waiting`, already the room's colour for "waiting on someone" — and a paused round is the room waiting on the host. The word carries it for anyone who cannot see the tint, and `aria-live` goes quiet, because there is nothing to announce when nothing is counting |
...
| `Keycap` | atom | One character drawn as a key — `sm` for the header hint, `lg` for the flash. Raised rather than flat: `$shadow-glass`'s inset top highlight is the whole difference between a box with a letter in it and something that looks pressable, and it was already in `theme/` for the landing page's frosted pill. Decorative by default — two caps read aloud as "⌥, P" teach nobody anything, so the caller hides them and writes the shortcut into a sentence |
| `ShortcutHint` | molecule | "Press ⌥ P to pause", beside the clock it pauses. Draws nothing without **both** a keyboard and room for one: `useHasKeyboard` asks `(hover: hover) and (pointer: fine)` because a host in landscape has a wide screen and no keys, and the stylesheet asks for `xl` because a narrow laptop window has keys and no room. The glyph is `⌥` on Apple platforms and `Alt` elsewhere; the visually hidden sentence says "Alt" either way, since that is the key's name. The second cap is a prop: `P` by default, `↵` for the host's advance in a paced room (ADR 0043) — with `capLabel` spelling a glyph that reads aloud as nothing |
| `ShortcutFlash` | molecule | The host's keystroke and what it did, centred over the room for `$duration-keyflash` — two keycaps over `Paused` or `Resumed`. **Fired by the state transition, not the keypress**: the two are almost the same instant and not quite, so a flash triggered by the press would have to guess which way it went and guess wrong for a frame, in a word the size of a headline. Watching the clock instead makes the label right by construction — and means the toolbox button gets the same confirmation, which reads as teaching the shortcut. `aria-hidden`; `TimerPill` carries the same fact in the accessibility tree and is the one that persists. Unmounts on a timer, never `animationend`, because reduced motion replaces the animation and the event never fires |
| `PlayerRow` | molecule | One player in a list — `roster`, `tracker`, `standing` or `pill`. The guest lobby's `pill` draws its avatar at 34px rather than the design's 30, because 30 is under `HAT_MIN_SIZE` and "who else is here" is the question a hat answers. A bot's badge comes off the face rather than a prop, so no screen can forget it — a bot nobody labelled is a person as far as the room can tell. `standing` splits its right-hand side in two: `note` is the container-gated column (rounds won, or what a non-competitor was doing), and `delta` is the round's points, drawn under the running total and behind **no** query — the row's two luxuries are the bar and the note, and the round's own number is neither. `as="li"` when the caller is a real list |
...
| `Modal` | molecule | The multi-step card — or a single-step announcement, which hides the step counter rather than reading "Step 1 of 1" (the podium's "Nobody paid the GIF bill" is one). 880px and one fixed height at every step, so it does not resize under a Next click. Its rail is a `ReactNode`, not a `src` — full-bleed, 380px beside the copy above `md` and a band above it on a phone, so Back and Next stay last. Three ways out — the close key, Escape, and a click on the backdrop, which ignores a selection drag that merely ended out there. A step's `body` is a `ReactNode`, not a string: the licensing walkthrough cites four licences and a repository, and a licence you cannot open is a licence nobody read. A step takes `body` (inline, in a `<p>`) **or** `bodyBlock` (block, in a `<div>`) — a prop rather than a second modal, because block markup inside the paragraph is not merely invalid but reparented by the browser, which takes the card's layout with it. **The body scrolls, not the card** — when the card was the scroller a long step pushed the foot below the fold, so the dots, Back and Next were unreachable and the copy slid up under the close key on its way past. `size` picks the measure: `step` is the design's walkthrough height, `list` the taller card a changelog needs. A one-step modal drops the step count, the dot row **and** Back: one dot is a bullet rather than progress, and a Back that can never enable is furniture beside the way out. Disabled is for a control that will become available — Back on step 1 of 4 — not for one that never will |
...
| `GifPanel` | molecule | GIF search, as a `popover` above the composer or a full-page `board`. Picking attaches and, in the popover, closes; it never sends. Masonry columns, each tile at its GIF's own ratio, lazily loaded and preferring the WebP rendition — a board is fifty tiles. `onMore` adds "Shuffle results" beside the suggestion chips, which fetches the next page for the same query and wraps at the end; `provider` drives the attribution mark, which lives here rather than on each screen so a new board cannot ship without it, and never appears over the offline shelf — it also turns off local keyword narrowing, because filtering a provider's results is against their terms. **While a board is out the last one comes down** — placeholder tiles at its own ratios, `TvStatic` alone, `aria-busy` on the panel — because a stale tile that is still clickable submits a GIF from the query you abandoned; an error keeps the last good board under the message and the retry key. `taken` marks the GIFs other players have already locked in: the vote grid's own-entry look, a `Taken` pill, `aria-disabled` and an inert handler, the tile still a real button in the tab order. Marked and never removed, because both providers forbid filtering a board, and nobody is named on it — entries are anonymous until the reveal |
...
| `LicenseModal` | molecule | "What this is built on" — the licences a production deploy carries: MIT for the app, the GIF providers' terms, CC0 for the faces, CC BY for the reaction art OFL for the type and CC0 for the music and sound effects. A configured `Modal` like `HelpModal`, five steps rather than one wall of text, because the card is a fixed height and these are genuinely different obligations to different parties. Kept by hand, not generated from `package.json`: what carries a condition is the *assets*, and a dependency list buries those under sixty MIT runtimes that ask for nothing |
...
| `ModeCard` | molecule | One of the two game modes, as a card with the sentence that explains it — a format, not a setting. Also the three bot levels in `BotPicker`, for the same reason: a level needs a sentence to be a choice rather than a label |
| `BotPicker` | molecule | **Not in this gallery** — one caller, and it is covered by `e2e/bots.spec.ts` instead. How ruthless a hired bot is, and the room's spend against its budget. Not a configured `Modal` — that one is a paged walkthrough whose footer a caller cannot replace, and this is a form. Renders as a **sibling** of any query container, never inside one: a container is the containing block for `position: fixed`, so a nested backdrop covers the column instead of the room |
...
| `WhatsNewModal` | molecule | The front door's changelog, one release per `Modal` step — so Back, Next, the dot row and the count all come free, and a reader who only wants the newest gets it first. The body is GitHub's own rendered HTML via `bodyBlock`, because release notes are headings and lists and the card's default body is a `<p>`. Three states that must never share a sentence: nothing shipped yet, GitHub unreachable, GitHub rate-limiting — an empty changelog and a broken one are different facts. Never `tone="error"`: the red glow is the room breaking under somebody mid-game, not a page that could not fetch a changelog |
...
| `SoundOffer` | molecule | The lobby asking, once, whether you want sound — host and guest alike, because rooms are remote-first and each person hears only their own device. **New** rather than a `Snackbar` or a `ChatToast`: it is a card with a title, a reason and two answers, and it lives in the lobby's layout rather than on the dock. **One row on a phone** — mark, title, "Turn sound on", and a `CloseButton` labelled "Keep it quiet" — **and a card above `md`** with the reason and a text decline: the phone's host lobby is packed to the pixel, and a four-line card pushed the mode toggle under the floating chat key. It sits first in the host's column for the same reason; anything low there opens half under the pinned start button. "Turn sound on" is `secondary` because the host's lobby already has its one primary. `onAccept` must unlock audio synchronously — the tap is the gesture the browser wants. Either answer stores `offered`, and the toolbox is where you change your mind |
...
6. **Timers are honest** — phases auto-advance at zero; ≤15s turns the pill red.
   A host-paced room keeps the rule by removing the timer rather than freezing
   it: `durationFor` returns `null`, `enterPhase` idles the clock and the header
   slot draws the host's advance control instead of a `0:00` that means nothing
...
| Scoreboard: one right-hand column per row, reading *"+4 this round"* or *"2 rounds won"* | Two. The note column keeps *"2 rounds won"*; the round's points move under the running total and shorten to *"+4"* | Screen 1m's own note promises "per-round deltas **and** rounds won", which the single column cannot keep — it reported one or the other, so a leader's round points were the one number on the board nobody could read. Worse on a phone: the column is behind a 560px container query measured on the row's own width, so the delta had never rendered there for anyone. Moving it into the score cell puts it behind no query at all. It loses the words "this round" because its new position says that for it — it sits under the running total, and the two numbers are the round and the game. Screen readers still hear them: the suffix is `srOnly`. Whoever set the round up gets an em dash rather than `+0`, since they were never allowed to score, and their note reads "Set this round up" — the same contextual idiom the design's own column uses for "picks next GIF" and "skipped a round" |
...
| Bot | A player the host hired rather than one who joined. Plays a full game — picks, captions, votes — and is labelled as one on every device, always |
| Intern / Senior / Principal | How ruthless a bot is — and **the ladder runs the other way to the seniority**. An intern has not learned what they cannot say; a principal has been media-trained. Hiring a Principal is the easy setting. The names are in career order because that ordering is the joke |
