...
| `AvatarPicker` | molecule | Choosing a face on `/join` and `/host`. Offers a window of ten from the seventy-seed catalogue, laid out five and five, with a "Shuffle faces" that re-rolls the offer and keeps your pick. A real radiogroup with roving tabindex and arrow keys. Owns the seed-to-preview-colour mapping so two screens cannot drift |
