...
| `ShortcutFlash` | molecule | The host's keystroke, shown for `$duration-keyflash` and gone. Sits under the clock rather than centred, so the thing that flashes is beside the thing that changed, and clears the docked rail via `--room-rail-width`. `aria-hidden` — `TimerPill` is what announces the state. Unmounts on a timer, never `animationend`, because reduced motion replaces the animation and the event never fires |
